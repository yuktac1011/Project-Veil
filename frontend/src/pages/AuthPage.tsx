...
export const AuthPage = () => {
  ...
  useEffect(() => {
    if (anonAadhaar.status === "logged-in") {
      ...
      // Extract Nullifier from the proof
      // @ts-ignore: Accessing internal structure if types are missing
      const proofs = (anonAadhaar as any).anonAadhaarProofs;
      let nullifier = "mock-nullifier-for-fallback";

      if (proofs) {
        // Taking the most recent proof
        const latestInfo = Object.values(proofs)[0] as any;
        // Depending on SDK version, nullifier might be in different spots
        // Usually it's in the proof 'claim' or 'publicSignals'
        // For Anon Aadhaar v2, we check extracting logic:
        nullifier = latestInfo?.nullifier || latestInfo?.proof?.nullifier || latestInfo?.publicSignals?.[1] || "fallback-nullifier";
        // Note: publicSignals[1] is often the nullifier in some circuits, but named property is better.
      }

      // If we still don't have a good nullifier, we use a random fallback but warn
      // In production, we should fail if no nullifier.
      console.log("Using Nullifier:", nullifier);

      const identity = deriveIdentityFromNullifier(String(nullifier));

      // Auto-create identity in your store with a default pin or prompt user
      // Using a default pin for the demo flow to reduce friction
      createIdentity(identity, "123456");
      navigate("/dashboard");
    }
